...
Future<MovieDataResponse?> movieDataResponseApi() async {
  String dataUrl = "${Constants.BASEURL}/erik-sytnyk/movies-list/master/db.json";
  var dioClient = Dio();
  ...
  dioClient.options.headers['Access-Control-Allow-Headers'] = {"Origin,Content-Type,X-Amz-Date,Authorization,X-Api-Key,X-Amz-Security-Token,locale"};
  dioClient.options.headers['Access-Control-Allow-Methods'] = "POST, GET, OPTIONS, PUT, DELETE, HEAD";
  try {
    final response = await dioClient.get(dataUrl);
    var box = Hive.box('movieDataList');
    Map<String, dynamic> result = jsonDecode(response.data);
    box.put(Constants.APIRESPONSE, jsonEncode(MovieDataResponse.fromJson(result)));
